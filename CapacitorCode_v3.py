...
'''
def GenerateInterdigitatedCapacitorCell_old(channel, GPLayer, TopLayer, OxideLayer, frequency, C, 
                                        via_pad_width, tolerance):
    IDC_name = "IDC_" + str(channel)
    IDC_cell = gdspy.Cell(IDC_name)
    
    # CALCULATE CAPACITOR SIZE - TO BE DONE BASED ON FREQUENCY, USE PLACEHOLDERS FOR NOW
    C.width = 1000  # placeholder!
    C.line_height = 800 # placeholder
    num_lines = int( C.width/ (C.line_width + C.gap_width) )
    # CREATE BASE
    x1 = 0
    y1 = 0
    x2 = C.width
    y2 = C.base_height
    bot = gdspy.Rectangle( (x1, y1), (x2, y2), **TopLayer) 
    IDC_cell.add(bot)
    ...
    y1 = C.base_height + C.gap_width + C.line_height
    y2 = y1 + C.base_height
    top = gdspy.Rectangle( (x1, y1), (x2, y2), **TopLayer) 
    IDC_cell.add(top)
    ...
    for i in range(num_lines):
        ...
        # ADD GEOMETRY
        line = gdspy.Rectangle( (x1, y1), (x2, y2), **TopLayer) 
        IDC_cell.add(line)
...
"""
def main():
    print("\n\n\nWelcome! Let's calculate some capacitances :) ")
    # GENERAL PROPERTIES
    c = 3.0e8 # speed of light
    
    
    # DESIRED CAPACITANCE PROPERTIES
    L_lumped = 60e-6 # inductance [Henry]
    freq_array = numpy.linspace(1.5e6, 1.5e6 + 99*40e3, 100)  # frequency schedule [Hertz] 
    nfreq = len(freq_array)
    #freq_array = [1.5e6, 2.02e6, 2.5e6, 3.02e6, 3.5e6, 4.02e6, 4.5e6, 5.02e6, 5.5e6] #, 3.5, 4.5, 5.5]
    #N_array =    [416 *2, 240*2, 187*2, 163*2,  152*2, 144*2,  114*2, 91*2,   77*2]
    #l_um_array = [4000,   3815,  3195,  2510,   2000,  1595,   1610,  1620,   1595]
    
    #substrate properties - Silicon
    er = 11.7       # dielectric constant
    h = 675 * 1e-6  # (m) height of the substrate material
    
    # geometry properties
    N = 410 * 2         # number of fingers
    l = 4000 * 1e-6     # (m) length of the interdigital capacitor finger
    l_um = l * 1e6      # (um) length of the interdigital capacitor finger
    W = 4 * 1e-6        # (m) width of the conductor
    S = 4 * 1e-6        # (m) width of the gap
    
    # CALCULATE INTERDIGITATED CAPACITOR PARAMETERS
    N_array = numpy.zeros(nfreq)
    l_um_array = numpy.zeros(nfreq)
    
    for f in range(nfreq):
        freq = freq_array[f]
        N, l_um = CalculateIDCapacitorParameters(freq, L_lumped, h, W, er)
        N_array[f] = N
        l_um_array[f] = l_um
    
    #CompareFrequencies(L_lumped, freq_array,  N_array, l_um_array, h, W, N, er)
